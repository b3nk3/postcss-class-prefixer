...
async function run(input: string, output: string, prefixeSelector = '') {
  const result = await postcss([plugin(prefixeSelector)]).process(input, {
    from: undefined,
  });
